...
public class Bishop {
    ...
    public Bishop(Context context, int pic, int id, float initialX, float initialY, float finalX, float finalY) {
        this.finalX = finalX;
        this.finalY = finalY;
        this.possX = possX;
        //this.possY = possY;
        ...
        bishop = BitmapFactory.decodeResource(context.getResources(), pic);
    }
    ...
    public void draw(Canvas canvas, int marginX, int marginY,
        ...
        canvas.translate(-bishop.getWidth() / 2f, -bishop.getHeight() / 2f);

        // Draw the bitmap
        canvas.drawBitmap(bishop, 0, 0, null);
        canvas.restore();
    }
    ...
    public boolean hit(float testX, float testY,
        ...
        if(pX < 0 || pX >= bishop.getWidth() ||
                pY < 0 || pY >= bishop.getHeight()) {
            return false;
        }

        // We are within the rectangle of the pawn.
        // Are we touching actual picture?
        return (bishop.getPixel(pX, pY) & 0xff000000) != 0;
    }
